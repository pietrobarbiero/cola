...
def main():
	...
	for nf in n_features:
		for ni in n_informative:
			# if ni <= nf:
			#     print(f'{nf} {ni}')
			experiments[f'f_{nf}_i_{ni}'] = [nf, ni]
	...
	for j, (dataset, data) in enumerate(progress_bar):
		...
		for ns in progress_bar_2:
			...
			plt.ylabel('Q')
			plt.title(f'{dataset}_s_{ns}')
			plt.legend(loc='upper center', bbox_to_anchor=(0.5, -0.3), ncol=2)
			plt.tight_layout()
			plt.savefig(os.path.join(results_dir, f'{dataset}_s_{ns}_loss_q.png'))
			plt.savefig(os.path.join(results_dir, f'{dataset}_s_{ns}_loss_q.pdf'))
			plt.show()
